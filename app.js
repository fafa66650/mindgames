...
const THEMES = {
  "": {body:'theme-night', title:'FAFATRAINING GAME ARENA', subtitle:'Une plateforme d’animation immersive pour préparer des escape games, répartir les équipes et lancer des missions visuelles, rythmées et adaptables selon ton public.'},
  night: {
    body:'theme-night', title:'FAFATRAINING GAME ARENA', subtitle:'Une mission urbaine sous pluie et néons, pensée pour la lecture d’indices, la logique et la tension.',
  ...
  jungle: {
    body:'theme-jungle', title:'FAFATRAINING GAME ARENA', subtitle:'Une expédition au cœur de ruines vivantes, entre repères cachés, trajectoires et glyphes.',
    mission:'Une ancienne voie est de nouveau ouverte. Mais les bons passages se lisent dans l’ordre, la forme et les détails.',
  ...
  lab: {
    body:'theme-lab', title:'FAFATRAINING GAME ARENA', subtitle:'Un protocole d’urgence dans un environnement instable, où chaque action, chaque lecture et chaque décision comptent.',
...
function save(){ localStorage.setItem('fafa_v10', JSON.stringify({...state, soundEnabled})); }
function load(){
  try {
    const raw = localStorage.getItem('fafa_v10');
    if(raw){
...
function clearStore(){ localStorage.removeItem('fafa_v10'); }
...
function renderAdmin(){
  ...
  APP.innerHTML = `
    <div class="hero">
      ...
      <div>
        <div class="kicker">BIENVENUE DANS FAFATRAINING</div>
        <h1>${theme.title}</h1>
        <div class="subtitle">${theme.subtitle}</div>
        <div class="notice"><strong>Ce que tu prépares ici :</strong><br>une expérience d’escape game mobile et ordinateur, avec équipes, rotation des joueurs, défis variés, chrono, cartes d’aide et ambiance de mission.</div>
        ${state.theme ? `<div class="notice"><strong>Univers choisi :</strong><br>${THEMES[state.theme].mission}</div>` : ''}
      </div>
      <div class="stats">
    ...
    <section class="panel">
      ...
      <div class="summary-grid" style="margin-top:14px">
        ...
        <div class="card"><h4>Audio</h4><div>${soundEnabled ? 'activé automatiquement' : 's’activera au lancement de mission'}</div></div>
      </div>
...
function launchMission(){
  ...
  state.hints=0; state.time=Number(state.duration); state.index=0; state.activeTeam=0; state.fragments=[]; state.metaPick=null; state.selectedChallenges=chooseChallenges(); state.timerActive=false; state.picked=null; state.attempts=0; state.showVisual=false;
  save();
...
function showIntro(isPlayer=false){
  const theme = THEMES[state.theme];
  playFile('glitch');
  APP.innerHTML = `<section class="cinematic"><div class="cinematic-box"><div class="kicker">OUVERTURE DE MISSION</div><h1>${state.theme==='night'?'Night Protocol':state.theme==='jungle'?'Expédition interdite':'Zone 13'}</h1><div class="notice"><strong>Tu n’entres pas dans un simple quiz.</strong><br>Tu entres dans une mission où l’équipe doit lire, observer, choisir, douter et avancer sous pression. Une erreur peut ralentir. Un bon choix peut relancer la partie.</div><div id="cineBox"></div><div class="btns"><button class="btn-main hidden" id="introBtn" onclick="${isPlayer ? 'goPlayer()' : 'goAdmin()'}">${isPlayer ? 'COMMENCER POUR L’ÉQUIPE' : 'ENTRER DANS LA PREMIÈRE ÉPREUVE'}</button></div></div></section>`;
...
function visualBlock(c){ return state.showVisual ? `<div class="visual"><img src="./assets/${c.visual}" alt="${c.visual}"></div>` : ''; }
function renderChallenge(showAdmin){
  ...
  const main = `
    <section class="game-shell fadein">
      ...
      <div class="rules"><strong>Règle :</strong> lis la scène, réponds, puis utilise une carte seulement si nécessaire.</div>
      ...
      <div class="visual-toggle"><button class="btn-alt" onclick="toggleVisual()">${state.showVisual ? 'MASQUER L’IMAGE' : 'AFFICHER L’IMAGE D’INDICE'}</button></div>
      ${visualBlock(c)}
  ...
  const monitor = showAdmin ? renderAdminMonitor() : '';
  APP.innerHTML = `<div class="game-layout">${main}${monitor}</div><div class="validate-bar"><button class="validate-btn" onclick="validate(${showAdmin})">VALIDER LA RÉPONSE</button></div>`;
}
...
function renderFinal(showAdmin){
  ...
  const main = `<section class="game-shell fadein">
  ...
  const monitor = showAdmin ? renderAdminMonitor() : '';
  APP.innerHTML = `<div class="game-layout">${main}${monitor}</div><div class="validate-bar"><button class="validate-btn" onclick="validateMeta()">VALIDER LE VERDICT FINAL</button></div>`;
}
...
function finishMission(timeout){
  ...
  APP.innerHTML = `
    ...
    <section class="final-card">
      ...
      <div class="grid3" style="margin-top:14px">${ranking.map((team,idx)=>`<div class="card"><h4>${idx+1}. ${team.name}</h4>Score : ${team.score} pts<br>Progression : ${team.progress}/${state.selectedChallenges.length}<br>Joueurs : ${team.players.filter(Boolean).length}</div>`).join('')}</div>
...
function resetAll(){
  clearStore();
  soundEnabled = false;
  state = {theme:'', totalPlayers:'', teamCount:'', publicType:'', difficulty:'', duration:'', teams:[], activeTeam:0, hints:0, time:0, index:0, picked:null, attempts:0, selectedChallenges:[], fragments:[], timerActive:false, metaPick:null, showVisual:false};
  location.hash = '#admin';
  renderRoute();
}
...
function drawRain(){
  ...
  CTX.strokeStyle='rgba(123,210,255,.35)';
  CTX.lineWidth=1;
  CTX.beginPath();
